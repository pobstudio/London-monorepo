...
import { getMinterTypeFromTokenId } from '../../stores/tokens/saga';
...
export const handleSagaMetadata = async (id: string, res: NextApiResponse) => {
  ...
  const mintingType = getMinterTypeFromTokenId(
  ...
  res.status(200).json({
    name:
      titleDocumentRes.writer !== NULL_ADDRESS
        ...
        : `${shortenHexString(paddedHashStr, 6)}`,
    description:
    ...
    algorithmStoredTxHash: deployments[CHAIN_ID].saga,
    properties: {
      ...attributes,
      season: {
        name: 'Season',
      ...
      from: {
        name: 'From',
        value: gene.from,
        display_value: shortenHexString(gene.from),
      },
      to: {
        name: 'To',
        value: gene.to,
        display_value: shortenHexString(gene.to),
      },
    },
