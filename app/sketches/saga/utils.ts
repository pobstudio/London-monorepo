import { Vec2, Cord, Rect, convertHexToRGBA } from '../../../utils/src';

export const cordFactoryUtils = {
  getRect: (rect: Rect, stepSize: number = 1) => {
    const cords: Cord[] = [];
    for (let i = rect[0][0]; i <= rect[1][0]; i += stepSize) {
      for (let j = rect[0][1]; j <= rect[1][1]; j += stepSize) {
        cords.push([i, j]);
      }
    }
    return cords;
