...
export const sketch = (
  ...
  const parallax = {
    depthStrength: 0.2,
    rotationStrength: 0.0001,
  };

  const createEllipseQuartersCommand = (delta: [number, number]) => {
    ...
    const command = regl({
      ...
      vert: `
                        ...
                        const vec3 X_AXIS = vec3(0.0, 1.0, 0.0);
                        const vec3 Y_AXIS = vec3(1.0, 0.0, 0.0);
                        ...
                        void main () {
                          ...
                          vec2 normalizedCords = vec2(2, 2) * ((position + (parallaxStrength * depthStrength * random(center) * (z + 1.0) * -1.0)) / resolution);
                          normalizedCords *= vec2(1, -1);
                          normalizedCords += vec2(-1, 1);
                          vec3 pos = rotate(rotate(vec3(normalizedCords, 0), X_AXIS, 1.0 * rotationStrength * parallaxStrength[0]), Y_AXIS, 1.0 * rotationStrength * parallaxStrength[1]);
  ...
  return {
    ...
    render: (delta: [number, number] = [0, 0]) => {
      const drawEllipseQuarters = createEllipseQuartersCommand(delta);
