...
export const useAccountCollection = (account?: string) => {
  ...
  useEffect(() => {
    ...
    updateCollectionMetadataMap((update) => {
      update[account] = {
        name: `${shortenHexString(account)} Collection`,
        seoName: `${shortenHexString(account)} Collection`,
        // description: `${data.hashOrIds.length} ${maybePluralizeWord(
        //   'artwork',
        //   data.hashOrIds.length,
        // )}`,
        description: `View your minted $HASH artwork, list it on OpenSea, or add an annotation to show your transaction’s history.`,
        type: 'minted',
        displayType: 'grid',
      };
    });
